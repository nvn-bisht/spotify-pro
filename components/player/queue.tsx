...
import { TilePlayPauseButton } from "../shared/Play-pause-button";
...
export function Queue() {
  ...
  return (
    <Sheet>
      ...
      <SheetContent
        dir="right"
        className="flex bg-neutral-900 backdrop-blur-md border-none flex-col space-y-2 px-2 md:max-w-xl"
      >
        <SheetHeader className="space-y-0 px-4 ">
          <SheetTitle className="flex items-center justify-between pr-4">
            <span className="font-heading text-2xl capitalize tracking-wide drop-shadow-md bg-clip-text  sm:text-3xl md:text-4xl text-white ">
              Queue
            </span>
          ...
          <ol className="space-y-2 text-muted-foreground">
            {queue.map((item) => (
              <li key={item.id} className="w-full">
                <div className="group flex h-14 w-full cursor-pointer items-center border-none justify-between truncate rounded-md  px-2 text-sm transition-shadow duration-150 hover:shadow-md">
                    ...
                    <figcaption className="flex flex-col">
                      <h4 className="w-full truncate font-semibold">
                        <Link
                          ...
                          className="text-white group-hover:text-neutral-200 "
                        >
                          {item.name}
                        </Link>
                      </h4>
                      ...
                      <ScrollArea className="w-full max-w-[400px] pb-1">
                        {item.artists.map((artist, i, arr) => (
                          <Link
                            ...
                            className="w-full truncate text-neutral-400 hover:text-neutral-200"
                          >
                            {artist.name}
                            {i !== arr.length - 1 && ", "}
                          </Link>
